...
lora_output_dir = "./saves/math_sft_train"
model_tokenizer = AutoTokenizer.from_pretrained(lora_output_dir)
...
print("Custom LoRA adjustments onto the base model...")
model = PeftModel.from_pretrained(base_model, lora_output_dir)

model.eval()
print("Ready! Your model now possesses its new math training.")
...
def model_get_response(conversation: List[Dict]):
    ...
    raw_response = model_tokenizer.decode(
        output[0][input_len:].tolist(), skip_special_tokens=False
    )
